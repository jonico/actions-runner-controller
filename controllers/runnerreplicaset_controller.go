	...
	if available > desired {
		...
		for _, runner := range myRunners {
			...
			if err != nil {
				...
				if errors.As(err, &e) {
					log.V(0).Info("debug", "Failed to check if runner is busy. Either this runner has never been successfully registered to GitHub or has not managed yet to, and therefore we prioritize it for deletion", "runnerName", runner.Name)
					notRegistered = true
				} else {
